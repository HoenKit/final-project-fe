...
namespace final_project_fe.Pages.Admin.ReportManager.CourseReport
{
    public class DeleteReportModel : PageModel
    {
        ...
        public GroupedReportDto<int, ReportCourseDto> GroupedReportCourses { get; set; }
        ...
        public async Task<IActionResult> OnPostDeleteReportAsync(int id)
        {
            ...
            try
            {
                ...
                string apiUrl = $"{_apiSettings.BaseUrl}/ReportCourse/DeleteReportsByCourseId/{id}";

                var response = await _httpClient.DeleteAsync(apiUrl);
                if (response.IsSuccessStatusCode)
                {
                    TempData["SuccessMessage"] = "Delete report successful.";
                    return RedirectToPage("/Admin/ReportManager/CourseReport/Index");
                }
                else
                {
