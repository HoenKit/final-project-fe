...
namespace final_project_fe.Pages.Admin.ReportManager.WorkshopsReport
{
    public class DeleteReportModel : PageModel
    {
        ...
        public GroupedReportDto<int, ReportWorkShopDto> GroupedReportWorkShops { get; set; }
        ...
        public async Task<IActionResult> OnPostDeleteReportAsync(int id)
        {
            ...
            try
            {
                ...
                string apiUrl = $"{_apiSettings.BaseUrl}/ReportWorkShop/DeleteReportsByWorkShopId/{id}";

                var response = await _httpClient.DeleteAsync(apiUrl);
                if (response.IsSuccessStatusCode)
                {
                    TempData["SuccessMessage"] = "Report deleted successfully.";
                    return RedirectToPage("/Admin/ReportManager/WorkShopsReport/Index");
                }
                else
                {
